const pageListadoMP = {
    init: function () {
        pageListadoMP.methods.ObtenerListas();
        pageListadoMP.methods.ListarMascotasPerdidas();
        pageListadoMP.sweetAlert.Init();

        $('#btn-buscar').click(pageListadoMP.events.btnBuscarClick);
        $('#btn-limpiar').click(pageListadoMP.events.btnLimpiarClick);
        $('#checkbox-reportar').on('change.bootstrapSwitch', pageListadoMP.events.changeCheckbox);
    ...
    methods: {
        ObtenerListas: function () {
            ...
            Promise.all([
                ...
                .then(pageListadoMP.methods.CargarListas)
        ...
        CargarListas: function ([pDataGeneroMascota, pDataColor, pDataTamanio, pDataRaza, pDataCola, pDataOrejas, pDataDistritos, pDataRedesSociales]) {
            ...
            pageListadoMP.data.listaGeneroMascota = pDataGeneroMascota == null ? [] : pDataGeneroMascota.map(x => x);
            pageListadoMP.data.listaColor = pDataColor == null ? [] : pDataColor.map(x => x);
            pageListadoMP.data.listaTamanio = pDataTamanio == null ? [] : pDataTamanio.map(x => x);
            pageListadoMP.data.listaRaza = pDataRaza == null ? [] : pDataRaza.map(x => x);
            pageListadoMP.data.listaCola = pDataCola == null ? [] : pDataCola.map(x => x);
            pageListadoMP.data.listaOrejas = pDataOrejas == null ? [] : pDataOrejas.map(x => x);
            pageListadoMP.data.listaDistritos = pDataDistritos == null ? [] : pDataDistritos.map(x => x);
            pageListadoMP.data.listaRedesSociales = pDataRedesSociales == null ? [] : pDataRedesSociales.map(x => x);
        ...
        ListarMascotasPerdidas: function () {
            let ID_Usuario = $('#ID_Usuario').val();
            if (ID_Usuario == "") {
                pageListadoMP.sweetAlert.swalDefaultInfo('Se perdio la sesion, por favor vuelva a ingresar.');
                return;
            }
            ...
            let vUrlListaMascotas = `https://localhost:44355/api/mascota/listar-mascotas-perdidas`;
            ...
            Promise.all([
                ...
                .then(pageListadoMP.methods.CargarTabla)


        },
        CargarTabla: function ([pDataMascotas]) {
            pageListadoMP.data.listaMascotas = pDataMascotas == null ? [] : pDataMascotas.map(x => x);
                ...
                },
                {
                    ...
                    'render': function (data, type, full, meta) {
                        ...
                        var buttom = '<div class="btn-group">'
                            ...
                            + '        <a class="dropdown-item" onclick="return pageListadoMP.methods.ObtenerDatosMascota(' + Id_Mascotas + ');" data-toggle="modal" data-target="#modal-reportar-mascota">Ver Detalle</a>'
                            + '    </div>'
                            + ' </div> ';
                        return buttom;
                    }
            ...
            $('#tblMascotas').DataTable({
                ...
                "autoWidth": false,
                "responsive": true,
                "language": pageListadoMP.data.DataTableLanguage,
                "columns": columns,
                "data": pageListadoMP.data.listaMascotas,
        ...
        ObtenerDatosMascota: function (IdMascota) {
            ...
            pageListadoMP.methods.ObtenerDetallesMascota(IdMascota);
            pageListadoMP.methods.ObtenerContactos(IdMascota);
        ...
        ObtenerDetallesMascota: function (IdMascota) {
            ...
            Promise.all([
                ...
                .then(pageListadoMP.methods.CargarDetalleMascota)
        },
        ...
        ObtenerContactos: function (IdMascota) {
            ...
            Promise.all([
                ...
                .then(pageListadoMP.methods.CargarContactos)
        },
        CargarContactos: function ([pDataContactos]) {

            pageListadoMP.data.vRedesSociales = pDataContactos == null ? [] : pDataContactos.map(x => x);
            ...
            var columnDefs = [
                {
                    ...
                    'render': function (data, type, full, meta) {
                        var buttom = "<center>" +
                            '<a class="btn btn-default btn-xs"  title="Eliminar" onclick="return pageListadoMP.methods.EliminarContacto(' + meta.row + ');"><i class="fa fa-trash" aria-hidden="true"></i></a>' +
                            "</center> ";
                        return buttom;
                    }
                }
            ...
            $('#tblContacto').DataTable({
                ...
                "language": pageListadoMP.data.DataTableLanguage,
                "columns": columns,
                "data": pageListadoMP.data.vRedesSociales,
                "columnDefs": columnDefs
            });
        },
    }
